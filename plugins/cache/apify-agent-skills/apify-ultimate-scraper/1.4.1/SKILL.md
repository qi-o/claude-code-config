...
当用户需要爬取小红书数据时，由于Apify没有小红书专用Actor，使用以下方案：

### 使用 XHS-Scraper (推荐)

**项目位置**: `C:/Users/ZDS/XHS-Scraper`

基于 Playwright 的小红书爬虫，需要用户扫码登录。

```bash
# 使用新版爬虫脚本
cd C:/Users/ZDS/XHS-Scraper
python xhs_scraper.py -q "搜索关键词" -n 5 -c 3 -o output.json
```
...
def scrape_xiaohongshu(query, num_notes=5):
    results = []
    with sync_playwright() as p:
        ...
        print("请扫码登录...")
        page.wait_for_selector('input[placeholder*="搜索"]', timeout=120000)

        # 搜索
        page.query_selector('input[placeholder*="搜索"]').fill(query)
        page.press('input[placeholder*="搜索"]', 'Enter')
        page.wait_for_timeout(5000)

        # 获取笔记
        notes = page.query_selector_all("section")[:num_notes]
        for note in notes:
            note.click()
...
### User Preferences
- 用户偏好 Apify 因其支持更多平台类型

### Known Fixes & Workarounds
- Apify Actor 输入参数必须是复数形式，如 searchQueries 而非 searchQuery
- 首次运行显示 fetch failed 时需等待任务完成，状态会从 RUNNING 变为 SUCCEEDED，应轮询检查最终状态

### Custom Instruction Injection

使用 Apify 前先检查 Actor 文档确认正确的参数名